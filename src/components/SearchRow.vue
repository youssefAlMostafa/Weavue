...
const hasData = () => locationStore.lat !== 0 || cityStore.list.length > 0
</script>
...
<template>
  <div class="flex items-center gap-[10px] mb-7 flex-wrap relative">
    ...
    <!-- Search bar -->
    <div class="flex items-center gap-[10px] bg-[var(--paper)] border border-[var(--line)] rounded-full px-[18px] pl-[16px] py-[10px] flex-1 min-w-[280px] max-w-[520px] transition-colors duration-200 focus-within:border-[var(--ink)] relative">
      <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" class="w-4 h-4 text-[var(--muted)] shrink-0">
        <circle cx="11" cy="11" r="7"/><path d="m20 20-3.5-3.5"/>
      </svg>
      <input
        v-model="query"
        placeholder="Search a city, region, or airport code…"
        autocomplete="off"
        class="flex-1 bg-transparent border-0 outline-none text-[14.5px] text-[var(--ink)] placeholder:text-[var(--muted)]"
        @blur="closeSuggestions"
      />
      <span class="[font-family:var(--mono)] text-[11px] text-[var(--muted)] border border-[var(--line)] rounded-[6px] px-[6px] py-[2px] shrink-0">⌘ K</span>

      <!-- Suggestions -->
      <div v-if="showSuggestions || isSearching" class="absolute top-[calc(100%+8px)] left-0 right-0 bg-[var(--paper)] border border-[var(--line)] rounded-[16px] shadow-[0_8px_24px_-8px_rgba(26,24,20,.15)] overflow-hidden z-50">
    ...
    <!-- Use my location -->
    <button
      class="inline-flex items-center gap-2 bg-[var(--ink)] text-[var(--cream)] border border-[var(--ink)] rounded-full px-4 py-[10px] text-[13.5px] transition-all duration-200 hover:bg-black disabled:opacity-60 disabled:cursor-not-allowed whitespace-nowrap"
      @click="useLocation"
      :disabled="isLocating"
    >
    ...
    <Transition enter-active-class="transition-opacity duration-200" leave-active-class="transition-opacity duration-200" enter-from-class="opacity-0" leave-to-class="opacity-0">
      <div v-if="errorMsg" class="fixed top-4 left-1/2 -translate-x-1/2 z-[100] flex items-center gap-3 bg-[var(--ink)] text-[var(--cream)] text-[13px] px-5 py-3 rounded-full shadow-lg">
        {{ errorMsg }}
